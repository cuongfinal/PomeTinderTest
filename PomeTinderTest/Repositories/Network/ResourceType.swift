...
extension ResourceType {
        ...
        do {
            ...
            case let .requestParametersAndHeaders(bodyParameters,
                ...
                try configureParameters(bodyParameters: bodyParameters,
                                        ...
                                        request: &request)
            case let .requestParametersArray(bodyParameters, bodyEncoding, urlParameters):

                try configureParametersArray(bodyParameters: bodyParameters, bodyEncoding: bodyEncoding,
                                        urlParameters: urlParameters, request: &request)
            }
            return request
        } catch {
    ...
    private func configureParametersArray(bodyParameters: [Parameters]?,
                                     bodyEncoding: ParameterEncoding,
                                     urlParameters: Parameters?,
                                     request: inout URLRequest) throws {
        do {
            try bodyEncoding.encode(urlRequest: &request,
                                    bodyParametersArray: bodyParameters,
                                    urlParameters: urlParameters)
        } catch {
            throw NetworkError.encoding(error.localizedDescription)
        }
